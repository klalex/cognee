...
async def ingest_data(
    ...
    # semantics are simply two documents. A single retry is insufficient with
    # multiple API/worker processes because another contender can win the retry
    # too. Re-read with bounded exponential backoff until the committed row is
    # visible and the existing-data path is taken. File writes are content-
    # addressed, so re-running is idempotent.
